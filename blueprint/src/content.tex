...
In general, we should be able to complete many of the proofs by considering $\mathbb{N}$ to
be a group under $\oplus$. With this, proving that the kernel is $0$ is sufficient to prove
injectivity, which is easier in the injective case. Instead of using induction for most things,
we can also use the fact that $\{2^n\}_n$ and $\{2^{n+1} - 1\}$ each generate the group.
I might try this approach if I were to dig in deeper.
