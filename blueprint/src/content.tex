...
\begin{theorem}
    ...
    \lean{direct_is_xor_homomorphism}
    \leanok
...
\begin{theorem}
    ...
    \lean{direct_inverse_is_recursive_inverse}
    \leanok
...
\begin{lemma}
    ...
    \lean{Nat.exists_xor_add_one_eq_two_pow}
    \leanok
...
\begin{theorem}
    ...
    \uses{def:direct_gray_code,def:unit_step_sequence}
    \lean{alternate_direct_gray_code_proof}
    \leanok
    From this, we have an alternate proof that $x \oplus (x >>> 1)$ is a unit step sequence.
\end{theorem}

\begin{proof}
    \uses{thm:add_one_xor}
    \leanok
...
\begin{lemma}
    ...
    \uses{def:natural_numbers_vector_space}
...
\begin{proof}
    For any given $x \in \mathbb{N}$, we can find the $i$th bit and that
    gives us the $i$th basis coefficient. The reverse takes the support $s$
    and does $\sum_{i \in s} 2^i$.

    This mapping is bijective and bit test preserves $\oplus$.
\end{proof}
...
\begin{proof}
    We can construct an equivalence with the basis on $2^n$.

    Since $2^n = (2^{n+1} - 1) \oplus (2^{n} - 1)$ for $n > 0$ and $2^0 = 2^1 - 1$.
    For the reverse, we can do $2^{n+1} - 1 = 2^0 \oplus \cdots \oplus 2^n$.

    Now it only remains to prove that they are independent. Since
    $(\bigoplus_{i \in s} 2^i - 1)_{\max(s)} = 1$, we have that any nontrivial
    sum is nonzero.
\end{proof}
...
\begin{proof}
    \uses{thm:two_pow_xor_basis,thm:two_pow_sub_one_xor_basis,def:natural_numbers_vector_space}
...
\begin{theorem}
    ...
    \uses{def:gray_code,def:direct_gray_code}
    We can then more easily prove that $x \oplus x >>> 1$ is a Gray code.
\end{theorem}

\begin{proof}
    \uses{thm:add_one_xor,thm:bijective_gray_code_characterization}
...
\begin{theorem}
    ...
    \uses{def:computable_inverse,def:direct_gray_code}
